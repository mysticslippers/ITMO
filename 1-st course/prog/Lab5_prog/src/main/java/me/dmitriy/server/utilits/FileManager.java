...
public class FileManager{
    ...
    public static ArrayList<Ticket> readFile(){
        ...
        try(BufferedReader bufferedReader = new BufferedReader(new FileReader(filePath))){
            while((line = bufferedReader.readLine()) != null) {     //Поиск информации об объекте
                ...
                if (matcher.find()) {
                    ...
                    if (line.contains("/venueType")) {      //Заполнение коллекции и проверка на валидность
                        try {
                            if (UserInputManager.isNameValid(info.get(1)) &&
                                    UserInputManager.isCoordinatesValid(new Coordinates(Float.parseFloat(info.get(2)), Integer.parseInt(info.get(3))))
                                    && UserInputManager.isPriceValid(Float.parseFloat(info.get(5)))
                                    ...
                                    && UserInputManager.isVenueValid(new Venue(Integer.parseInt(info.get(9)), info.get(10), Long.parseLong(info.get(11)), VenueType.valueOf(info.get(12))))) {
    ...
    public static void writeFile(ArrayList<Ticket> collection){
        ArrayList<String> info = new ArrayList<>();
        String filePath = System.getenv("Collection1");
        ...
        try(FileOutputStream fileOutputStream = new FileOutputStream(filePath)){
            ...
            for (Ticket ticket : collection) {                  //Запись объектов коллекции в файл
                fileOutputStream.write(("\t").getBytes());
                xmlStreamWriter.writeStartElement(("Ticket"));
                fileOutputStream.write(("\n\t\t").getBytes());

                line = ticket.toString();                            //Поиск информации об объекте
                Matcher matcher = patternAttribute.matcher(line);
                while (matcher.find()) {
